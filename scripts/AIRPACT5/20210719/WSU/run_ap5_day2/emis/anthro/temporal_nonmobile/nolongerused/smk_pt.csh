...
# inventory source and general flags
  ...
  setenv SMK_EMLAYS    21                # number of layers
...
# was       setenv PTSUPNAME  ptsup_${SECTOR}_${INVSRC}_
     ...
     # run SMOKE's temporal program
       ...
       set exitstat = $status
       if ( $exitstat ) then
          echo "*** Error running temporal in smk_pt.csh ***"
          exit ( $exitstat )
       endif
  endif # RUN_TEMPORAL
...
# ELEVPOINT
  if ( $RUN_ELEVPOINT == 'Y' ) then
     # input switches
       setenv SMK_PING_METHOD    0 # 1 PING, 0 no PING
       setenv SMK_ELEV_METHOD    2 # 0=Laypoint sets elev srcs; 1=use PELVCONFIG 2= CMAQ inline processing for elevated sources used
       setenv ELEV_WRITE_LATLON  Y 
     # input files
     ...
     # execute program
       ...
       set exitstat = $status
       if ( $exitstat ) then
          echo "*** Error running elevpoint in smk_pt.csh ***"
          exit ( $exitstat )
       endif
  endif # RUN_ELEVPOINT
...
# LAYPOINT 
  if ( $RUN_LAYPOINT == 'Y' ) then
     # input switches
       setenv EXPLICIT_PLUMES_YN   Y     # explicit plume rise sources from PELV
       setenv SMK_SPECELEV_YN      Y     # Y: Laypoint uses Elevpoint outputs to pick elevated
     ...
     # input files
       ...
       setenv GRID_CRO_3D $METDIR/GRIDCRO3D
     ...
     # execute program
       ...
       set exitstat = $status
       if ( $exitstat ) then
          echo "*** Error running laypoint in smk_pt.csh ***"
          exit ( $exitstat )
       endif
  endif # RUN_LAYPOINT
...
# SMKMERGE
  if ( $RUN_SMKMERGE == 'Y' ) then
     ...
     # input files
      ...
      # output files
        setenv PGTS3D_L         $OUTDIR/pgts3d_l_${SECTOR}_${INVSRC}_${CJDATE}.ncf
...
#        setenv PGTS             $OUTDIR/pgts_m.${SECTOR}_${INVSRC}_${CJDATE}.ncf  # JKV WTF?
      ...
      # execute program
        ...
        set exitstat = $status
        if ( $exitstat ) then
           echo "*** Error running smkmerge in smk_pt.csh ***"
           exit ( $exitstat )
        endif
  endif # RUN_SMKMERGE
