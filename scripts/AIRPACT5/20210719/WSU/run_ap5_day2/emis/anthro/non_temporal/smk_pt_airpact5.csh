#!/bin/csh -f
#
# Version @(#)$Id: smk_pt_nctox.csh,v 1.8 2007/10/10 15:04:45 bbaek Exp $
# Path    $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/scripts/run/smk_pt_nctox.csh,v $
# Date    $Date: 2007/10/10 15:04:45 $
#
...
#   setenv SMK_NHAPEXCLUDE_YN   N   # Y uses NHAPEXCLUDE file when integrating toxic sources
  # JKV 052715  setenv SMK_PROCESS_HAPS     ALL # This is for SMOKE 2.7 ALL|NONE|PARTIAL
#   setenv SMK_PROCESS_HAPS     NONE # This is for SMOKE 3.5.1 JKV 052715  ALL|NONE|PARTIAL
...
#  setenv NHAPEXCLUDE $INVDIR/other/dummyname.txt
...
#
  ...
  source $ASSIGNS_FILE   # Invoke Assigns file to set new dates
  source $SCRIPTS/run/smk_run.csh     # Run programs
  source $SCRIPTS/run/qa_run.csh      # Run QA for part 3
  setenv RUN_PART3 N
...
## Loop through dats to run Laypoint and Smkmerge
  ...
  while ( $cnt < $EPI_NDAY )
     ...
     source $ASSIGNS_FILE   # Invoke Assigns file to set new dates
     source $SCRIPTS/run/smk_run.csh     # Run programs
     source $SCRIPTS/run/qa_run.csh      # Run QA for part 4
