...
#
source $ASSIGNS/set_dirs_fire.csh
if ( $status > 0 ) then
	echo BAD STATUS ON set_dirs_fire.csh JKV
endif
...
# Now in ver 072110 building lists of inventory files.  JKV
   setenv PTINV      $AIROUT/EMISSION/fire/bluesky/invent_ptinv.txt
   echo "#LIST ORL" >! $PTINV
   echo "$AIROUT/EMISSION/fire/bluesky/ptinv-${CurYr}${CurMn}${CurDt}00.orl" >> $PTINV
   setenv PTDAY     $AIROUT/EMISSION/fire/bluesky/invent_ptday.txt
   echo "#LIST ORL" >! $PTDAY
   echo "$AIROUT/EMISSION/fire/bluesky/ptday-${CurYr}${CurMn}${CurDt}00.orl" >> $PTDAY
