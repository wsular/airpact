...
# This script runs the SMOKE processors for fire emissions.
#
         ...
         if ( $debugmode == Y ) then
            ...
            if ( -e $SMK_BIN/grdmat ) then
               ...
               if ( `echo $tmp_str1` == `echo "No source-cell intersections found."` ) then
                  #echo "     no fires in airpact-4 domain according to SMARTFire/BlueSky -- run CMAQ without fires" >> $AIRLOGDIR/summary.log
                   echo "     no fires in airpact-4 domain according to SMARTFire/BlueSky " >> $AIRLOGDIR/emis/emis_canfire.log
	       	   exit(1)
	       else 
	       	    set tmp_str1 = `tail -3 "$TMPLOG"`
	       	    if ( `echo $tmp_str1` != `echo "--->> Normal Completion of program GRDMAT"` ) then 
                       echo "  last part of GRDMAT log file for fire processing "
                       echo $tmp_str1
	              #echo "     GRDMAT failed for BlueSky output files -- run CMAQ without fires" >> $AIRLOGDIR/summary.log
	               echo "     GRDMAT failed for BlueSky output files " >> $AIRLOGDIR/emis/emis_canfire.log
	               exit(1)
	            endif
	       endif
            else
