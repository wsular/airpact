...
# 
#   > run_emis_fire.csh 20120805
#
#
#  Serena H. Chung 2012-05-18
#  Farren L. Herron-Thorpe 2016-04-28
#
#  Description:
#
#       This script is models fire emissions using BlueSky.
#
#       Calling tree when running within the airpact-4 forecast operation:
#
#         qsub4precctm.csh -> run_emis_fire.csh
#
#  Precondition(s):
#
#     setenv AIRHOME
...
#> remove any files that might exist if final fire file does not exist; otherwise exit if it already exists
   setenv TARGET_DIR $AIROUT/EMISSION/fire/bluesky
   mkdir -v -p $TARGET_DIR

   if ( ! -e $AIROUT/EMISSION/fire/smoke/pgts3d_l_${YEARDOY}_1_AIRPACT_04km_fire.ncf ) then
      rm -r -f $AIROUT/EMISSION/fire/smoke
      rm -rf $AIRLOGDIR/emis/fire
   else
      echo "Found final fire emissions file"
      ls $AIROUT/EMISSION/fire/smoke/pgts3d_l_${YEARDOY}_1_AIRPACT_04km_fire.ncf
      exit(0)
   endif
...
#> check to see if inputs exist
   ls -altH $AIROUT/EMISSION/fire/bluesky/ptinv-${YMD}00.orl

#> Copy files from DAY1 so BlueSky doesn't have to run twice
   if ( ! -e $AIROUT/EMISSION/fire/bluesky/ptinv-${YMD}00.orl ) then
      #This simplified copy method will fail on January 1
      set DAY1 = `datshift $YMD -1`
      echo "Copying DAY1 fire info to DAY2"
#      cp -r /data//lar/projects/airpact5/AIRRUN/$SRTYR/${DAY1}00/EMISSION/fire/bluesky $AIROUT/EMISSION/fire/
      cp -r $AIRRUN1/${DAY1}00/EMISSION/fire/bluesky $AIROUT/EMISSION/fire/
      mv $AIROUT/EMISSION/fire/bluesky/ptinv-${DAY1}00.orl $AIROUT/EMISSION/fire/bluesky/ptinv-${YMD}00.orl
      mv $AIROUT/EMISSION/fire/bluesky/ptday-${DAY1}00.orl $AIROUT/EMISSION/fire/bluesky/ptday-${YMD}00.orl

   endif

#> End file copy

#> check to see if inputs exist, if not, exit
   if ( ! -e $AIROUT/EMISSION/fire/bluesky/ptinv-${YMD}00.orl ) then
      echo "No Fires From BlueSky Available"
      exit(0)
   else
      echo "ORL File Format Fires Found"
   endif
...
#> main SMOKE script --------------------------------------------------
   ...
   if ( `echo ${tmp_strb}` == `echo "#DESC FIPS"` ) then
      echo "Fire list is empty; stopping fire emissions process."
      exit(0)
   endif


${PBS_O_WORKDIR}/emis/fire_orl/run_smk_pt_fire.csh

#> If there is no final fire file check for no fires in domain in grdmat log
if ( ! -e $AIROUT/EMISSION/fire/smoke/pgts3d_l_${YEARDOY}_1_AIRPACT_04km_fire.ncf ) then

   set tmp_str1 = `tail -2 "$AIROUT/LOGS/emis/fire/smkinven_pt_fire.log"`
   set tmp_str2 = `echo ${tmp_str1} | cut -c1-43`

   set tmp_str5 = `tail -2 "$AIROUT/LOGS/emis/fire/grdmat_pt_fire_AIRPACT_04km.log"`
...
ls -al $AIROUT/EMISSION/fire/smoke/pgts3d_l_${YEARDOY}_1_AIRPACT_04km_fire.ncf
