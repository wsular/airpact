...
#> setup directory and file location -----------------------------------
   ...
   setenv ASSIGNS_FILE $EBASE/assigns/ASSIGNS_AIRPACT5_fire.csh # wildfire only as pt emis
...
#> run smkinven, spcmat, grdmat, cntlmat, if needed --------------------
   ...
   if ($RUN_SMKINVEN == 'Y' || $RUN_SPCMAT == 'Y' || $RUN_GRDMAT == 'Y' ) then
	...
	setenv NHAPEXCLUDE $INVDIR/other/nhapexclude.all.txt
      ...
      # run programs
        source ${PBS_O_WORKDIR}/emis/fire_orl/run_smk_fire.csh     # Run programs
        if ( $status != 0 ) then
...
#> run other subprograms -----------------------------------------------
   ...
   # temporal
	...
	set exitstat = $status
	if ( $exitstat ) then
	   echo "*** Error running TEMPORAL using run_smk_fire.csh ***"
	   exit ( $exitstat )
	endif
     endif # RUN_TEMPORAL
   ...
   # laypoint
	...
	set exitstat = $status
	if ( $exitstat ) then
	   echo "*** Error running LAYPOINT using run_smk_fire.csh***"
	   exit ( $exitstat )
	endif
     endif # RUN_LAYPOINT
   ...
   # smkmerg
	...
	setenv RUN_SMKMERGE  Y 
     ...
     if ( $RUN_SMKMERGE == 'Y' ) then
        ...
        set exitstat = $status
	if ( $exitstat ) then
	   echo "*** Error running SMKMERGE using run_smk_fire.csh***"
	   exit ( $exitstat )
	endif
