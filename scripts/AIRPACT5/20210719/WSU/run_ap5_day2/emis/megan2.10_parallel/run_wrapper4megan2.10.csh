...
#  2016-04-30    Serena H. Chung      uses parallelized emproc
...
# "pre"conditions --need to set during test ----------------------------
   ...
   if ( ! $?MCIPDIR        ) setenv MCIPDIR   $MCIPROOT/${currentday}00/MCIP37_2
   if ( ! $?PBS_O_WORKDIR  ) setenv PBS_O_WORKDIR  ~airpact5/AIRHOME/run_ap5_day2


# ----------------------------------------------------------------------
   setenv logdir $AIRLOGDIR/emis/megan2.10
   if ( ! -d $logdir ) mkdir -p $logdir
   $PBS_O_WORKDIR/emis/megan2.10_parallel/run_met2mgn.csh  $epiday  $currentday    >&! $logdir/log01_met2mgn.txt
   $PBS_O_WORKDIR/emis/megan2.10_parallel/run_emproc_parallel.csh   $currentday    >&! $logdir/log02_emproc.txt
   $PBS_O_WORKDIR/emis/megan2.10_parallel/run_mgn2mech.csh          $currentday    >&! $logdir/log03_mgn2mech.txt
