...
# 
#   > run_ncl_AQIcolors_24hrPM25.csh 20160201 24
#
#    where 20101104 denotes forecast start time of 8 GMT on Nov 5, 2010
#               24 denotes 24 hours of forecast
#
# Joseph K. Vaughan     2015-09-22
# Serena H. Chung       2012-05-14
# Farren L. Herron-Thorpe 2013-02-11
# This C shell script creates hourly AQI-colored gif files of 24-hr PM25 concentrations based on AIRPACT-4's
...
#> during testing 
   ...
   if ( ! $?PBS_O_WORKDIR ) then
     ...
     set AIROUT = /data/lar/projects/airpact5/AIRRUNDAY2/${yearm1}/${1}00
     set MCIPDIR   = /data/lar/projects/airpact5/AIRRUNDAY2/${yearm1}/${previousday}00/MCIP37
     set NHR = $2
   endif
...
cat >! $NCLSCRIPTS/AQIcolors_ave24hrPM25.ncl <<EOF
...
begin
  ...
  ; must include ".nc" in NCL script)
    f1 = addfile ( "$indir" + "/POST/CCTM/PM25_L01_24hr_${YYYYMMDDm1}.ncf.nc","r")
  ...
  ; ------------------------------------------------------------------
	...
	res@gsnPaperOrientation = "Portrait"
        ...
        AQIcolormap = (/"white", "black", "green4", "green1", "yellow1", "orange1", "red1", "maroon4", "firebrick4", "firebrick4"/)
        res@cnLevels             = (/8.0, 12.0, 35.5, 55.5, 150.5, 250.5, 350.5/)
  ...
  ; ------------------------------------------------------------------
    ...
    do ifilestep = 0, (nstep-1)
       ...
       outfile = "$PSDIR"+"/airpact5_AQIcolors_24hrPM25_"+YYYYMMDD2Use+ aPST
;       res@tiMainString    = "PM2.5 (24hr avg) " +YYYYMMDD2Use + "  " +aPST + " PST"
;       delete(res@cnLevels) 
...
EOF
# end of AQIcolors_ave24hrPM25.ncl

ncl -Q $NCLSCRIPTS/AQIcolors_ave24hrPM25.ncl
if ($status == 0 ) /bin/rm -f $NCLSCRIPTS/AQIcolors_ave24hrPM25.ncl
