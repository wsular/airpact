...
# 
#   > run_ncl_west_o3_curtain.csh 20120501 24
#
...
#
# This C shell script creates hourly gif files of western boundary curtain plots based on AIRPACT-4
...
setenv ihour $ihour
setenv ahour $ahour
cat > $NCLSCRIPTS/west_o3_curtain.ncl <<EOF
...
;********************************************************
  ; position and time to read
    ...
    location_description = "Western Boundary" ; some text to describe the location
  ; ozone concentration
...
;********************************************************
  ...
  res@tiMainString         = "AIRPACT-4 at " + location_description
  res@gsnLeftString        = ""
...
;********************************************************
  ; ozone
    wks = gsn_open_wks("ps" ,"$PSDIR"+"/airpact5_west_o3_curtain_" + "$ahour")          ; ps,pdf,x11,ncgm,eps
   ...
   ; and that the Y coordinates only range from -90 to 90.
     ...
     setvalues plot
       ...
       "tiMainString"  : "AIRPACT-4 Western Boundary"
  ...
  ; CO
    wks = gsn_open_wks("ps" ,"$PSDIR"+"/airpact5_west_co_curtain_" + "$ahour")          ; ps,pdf,x11,ncgm,eps
    gsn_define_colormap(wks,"BlGrYeOrReVi200")           ; select color map
...
EOF
# end of west_o3_curtain.ncl

       ncl -Q $NCLSCRIPTS/west_o3_curtain.ncl
      #rm -r -f $PSDIR/*.ps
       @ ihour ++
end #while
...
/bin/rm -f $NCLSCRIPTS/west_o3_curtain.ncl
exit
