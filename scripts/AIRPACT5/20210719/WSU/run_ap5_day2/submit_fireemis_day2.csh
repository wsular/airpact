...
#  if current time is during AM as expected, delay until 0130 AM (01:30).
#     set nowis = `date | cut -d' ' -f4 | cut -f1,2 -d':' | tr -d ':'`
     set nowis = `date | cut -d' ' -f4 | cut -f1,2 -d':' | tr -d ':' | sed -e 's/^0//'`
     set not_until = 0130  #  time to start fire emissions script.
#     if ( $nowis < 0600 ) then
     if ( $nowis < 600 ) then
        echo Time of run is in expected window of AM hours, so use delay option.
