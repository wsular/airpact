   ...
   #> input files and directories
     ...
     #set TR_EMfile = $EMISfile # 
      ...
      if ( ! -e $GC_ICpath/$GC_ICfile ) then
         ...
         echo "no previous day CGRID to use for ICON ..wait a few minutes"
	 sleep 300
         if ( ! -e $GC_ICpath/$GC_ICfile ) then
            echo $GC_ICpath/$GC_ICfile "not found"
            echo "no previous day CGRID to use for ICON ..wait a few minutes again"
   	    sleep 300
            if ( ! -e $GC_ICpath/$GC_ICfile ) then
               echo $GC_ICpath/$GC_ICfile "not found"
               echo "no previous day CGRID to use for ICON ..stop"
               exit(1)
            endif
         endif
      endif
