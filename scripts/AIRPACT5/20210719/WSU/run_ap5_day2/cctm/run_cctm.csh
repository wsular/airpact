...
#  Joe Vaughan Added loop for waiting on CGRID file  2016-12-26
# 
#  description:
#
#
...
#> input and output directories and files ------------------------------
   ...
   #> input files and directories
     ...
     #set TR_EMfile = $EMISfile # 
      set GC_ICpath = $AIRROOT1/${previousdayyear}/${previousday}00/CCTM
      set GC_ICfile = CGRID_${previousday}.ncf
      if ( ! -e $GC_ICpath/$GC_ICfile ) then
         echo $GC_ICpath/$GC_ICfile "not found"
	 ls -lt ${GC_ICpath}
         echo "no previous day CGRID to use for ICON ..Loop for up to 3 hours"  
	 set loop_cnt = 0
...
#	 	if ( ${loop_cnt} == 16 ) then
	 	if ( ${loop_cnt} == 32 ) then
#			echo Three-hour Loop Ending With No CGRID File, Exiting
			echo Six-hour Loop Ending With No CGRID File, Exiting
			exit(1)
	 	endif
         	sleep 900
	end
      ...
      set GC_BCpath = $AIROUT/BCON/output
...
# JKV 102815  Delete cctm.log and all logs under $AIRLOGDIR/cctm in ase of a rerun
  ...
  #setenv ACONC_END_TIME Y #> override default beginning ACON timestamp [ default: N ]
   ...
   #> inline plume rise emissions processing
      setenv CTM_PT3DEMIS N #shc switched to N on 2016-04-23
      # setenv NPTGRPS 1
      # set SECTOR = point
