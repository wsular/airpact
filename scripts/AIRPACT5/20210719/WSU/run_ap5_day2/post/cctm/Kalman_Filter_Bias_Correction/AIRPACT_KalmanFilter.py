...
date = pd.to_datetime('tomorrow').strftime('%Y%m%d')
year = pd.to_datetime('tomorrow').year
#yesterday = pd.datetime.strftime(pd.datetime.now() - datetime.timedelta(1), '%Y%m%d')
today = pd.datetime.strftime(pd.datetime.now() - datetime.timedelta(1), '%Y%m%d')
#time_select = pd.to_datetime('today').strftime('%Y-%m-%d')
time_select = pd.to_datetime('tomorrow').strftime('%Y-%m-%d')
...
#nc_file = '/data/lar/projects/airpact5/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00/POST/CCTM/PM25_L01_24hr_'+str(yesterday)+'.ncf'
nc_file = '/data/lar/projects/airpact5/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00/POST/CCTM/PM25_L01_24hr_'+str(today)+'.ncf'
mod_grid = Dataset(nc_file)
mod_vals = mod_grid.variables['RollingA24_PM25'][1,0,:,:]

grid_data_file = '/data/lar/projects/airpact5/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00/MCIP37/GRIDCRO2D'
grid_data = Dataset(grid_data_file)
LAT = grid_data.variables['LAT'][0,0,:,0]
LON = grid_data.variables['LON'][0,0,0,:]

# Output File Names
#datedir = '/data/lar/projects/KFBC/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00'
datedir = '/data/lar/projects/airpact5/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00'
try:
	os.mkdir(datedir)
except FileExistsError:
	pass
outdir = '/data/lar/projects/airpact5/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00/POST/CCTM'
imagedir = '/data/lar/projects/airpact5/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00/IMAGES/KFBC'
#try:
...
location_filepath = '/data/lar/projects/airpact5/AIRRUNDAY2/'+str(year)+'/'+str(date)+'00/POST/CCTM/aqsid.csv'
