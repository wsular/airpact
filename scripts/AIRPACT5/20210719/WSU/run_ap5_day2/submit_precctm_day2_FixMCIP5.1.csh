...
#  usage examples:
#   
#       ./submit_precctm_day2.csh 
#       ./submit_precctm_day2.csh <firstday in YYYYMMDD>
#       ./submit_precctm_day2.csh <firstday in YYYYMMDD> <runid2waiton>
# 
...
# estimate time required for running the job ---------------------------
  ...
  # (give a 50-100% buffer)
    @ totalminutes = 0
    if ( $RUN_BCON == "Y" )           @ totalminutes = $totalminutes + 3
    if ( $RUN_FIX_MCIP  == "Y" )      @ totalminutes = $totalminutes + 5 
    if ( $RUN_ICON_ideal == "Y" )     @ totalminutes = $totalminutes + 1
    if ( $RUN_JPROC == "Y" )          @ totalminutes = $totalminutes + 2
...
# job submission -------------------------------------------------------
  # create qsub script
    /bin/sed -f input4sed.txt < blueprints/blueprint_qsub_precctm_FixMCIP5.1.txt > $qsubfile
    echo `date` "submitting pre-cctm for" $secondday
