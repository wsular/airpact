...
# ----------------------------------------------------------------------
  ...
  # MEGAN
    ...
    if ( -e $logfile ) then
         ...
         if ( `echo $tmp_str2` != `echo "Normal completion by met2mgn"` ) then
            ...
            echo "      will not cleanup non CCTM files" >> $AIRLOGDIR/summary.txt
            exit(1) 
         endif
     else
        echo `date` >> $AIRLOGDIR/summary.txt
     ...
     set logfile = $AIRLOGDIR/emis/megan2.10/log_emproc_000.txt
     if ( -e $logfile ) then
        set tmp_str1 = `tail -4 $logfile`
        set tmp_str2 = `echo "$tmp_str1[2-4]"`
        if ( `echo $tmp_str2` != `echo "Program completed successfully"` ) then
           echo `date` >> $AIRLOGDIR/summary.txt
  ...
  # anthropogenic emission files
    set logfile = $AIRLOGDIR/emis/emis_merge_mrggrid.log
    if ( -e $logfile ) then
       set tmp_str1 = `tail -4 $logfile`
   ...
   if ( -d $AIROUT/EMISSION/anthro/output/all_other ) then
      /bin/rm -f  $AIROUT/EMISSION/anthro/output/all_other/*.ncf
   endif
   if ( -d $AIROUT/EMISSION/anthro/output/all_afdust ) then
      /bin/rm -f  $AIROUT/EMISSION/anthro/output/all_afdust/*.ncf
   endif
   if ( -d $AIROUT/EMISSION/anthro/output/all_nodust ) then
   ...
   # echo `date` >> $AIRLOGDIR/summary.txt
   # echo "  intermediate anthropogenic emission files removed   "  >> $AIRLOGDIR/summary.txt
   # echo "  ${fivedaysago} anthropogenic emission files removed "  >> $AIRLOGDIR/summary.txt
