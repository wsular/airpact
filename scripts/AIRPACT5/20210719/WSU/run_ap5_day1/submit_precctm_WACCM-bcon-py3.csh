...
# job submission -------------------------------------------------------
  # create qsub script
#    /bin/sed -f input4sed.txt < blueprints/blueprint_qsub_precctm_WACCM-bcon_FixMCIP5.1_py3.txt > $qsubfile
    /bin/sed -f input4sed.txt < blueprints/blueprint_qsub_precctm_WACCM-bcon-py3.txt > $qsubfile
    echo `date` "submitting pre-cctm for" $currentday
    if ( $?runid0 ) then # if waiting on a job to finish
       echo " queing behind" $runid0
       set qsubreturn = `qsub  -W depend=afterok:${runid0}  $qsubfile`
       unsetenv runid0
    else
       set qsubreturn = `qsub  $qsubfile`
    endif
