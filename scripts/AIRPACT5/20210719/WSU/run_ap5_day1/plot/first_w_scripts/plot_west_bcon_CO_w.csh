...
# 
#   > plot_west_bcon_CO.csh 20120501 24
#
...
# plot_west_bcon_CO.csh 20120501 24 notes:
# Joe Vaughan           2016-12-28 Modified for testing in AIRPACT5

# Old comments:
# This C shell script creates hourly gif files of plots of the carbon monoxide on the 
...
;********************************************************
  ; position and time to read
    ...
    location_description = "Western Boundary CO" ; some text to describe the location
  ; BEGIN READ IN species from bcon file
        ;       CO
        	CO_bcon = fbcon->CO
        	west_CO = CO_bcon(${ihour},:,832:1089)
  ; END READ IN bcon species

    west_CO       = west_CO *1000.                  ; convert to ppb
    ; define/modify dimensions and attributes in a way that NCL understands
  ;    west_CO!0 = "time"       ; first dimension
      west_CO!0 = "lev"       ; first dimension
      west_CO!1 = "south_north" ; second dimension
      west_CO@long_name = "CO MOZART BCON"
      west_CO@units = "ppb"

   ; get dimension sizes
     dimsiz = dimsizes(west_CO)
...
;********************************************************
;CO
  wks = gsn_open_wks("ps" ,"$PSDIR"+"/airpact5_BCON_west_CO_" + "w_$ahour")          ; ps,pdf,x11,ncgm,eps
  gsn_define_colormap(wks,"BlGrYeOrReVi200")           ; select color map
      res@lbTitleString         = "(" + west_CO@units + ")"
          plot  = gsn_csm_contour(wks,west_CO(:,:),res)
        draw(plot)
