...
# 
#   > plot_south_bcon_O3.csh 20120501 24
#
...
# plot_south_bcon_O3.csh 20120501 24 notes:
...
;********************************************************
  ; position and time to read
    ...
    location_description = "Southern Boundary O3" ; some text to describe the location
  ; BEGIN READ IN species from bcon file
        	O3_bcon = fbcon->O3
        	south_O3 = O3_bcon(${ihour},:,0:284)
  ; END READ IN bcon species

    south_O3       = south_O3 *1000.                  ; convert to ppb
    ; define/modify dimensions and attributes in a way that NCL understands
  ;    south_O3!0 = "time"       ; first dimension
      south_O3!0 = "lev"       ; first dimension
      south_O3!1 = "west_east" ; second dimension
      south_O3@long_name = "Ozone MOZART BCON"
      south_O3@units = "ppb"

   ; get dimension sizes
     dimsiz = dimsizes(south_O3)
...
;********************************************************
;OZONE
  wks = gsn_open_wks("ps" ,"$PSDIR"+"/airpact5_BCON_south_O3_" + "w_$ahour")          ; ps,pdf,x11,ncgm,eps
  gsn_define_colormap(wks,"BlGrYeOrReVi200")           ; select color map
      res@lbTitleString         = "(" + south_O3@units + ")"
          plot  = gsn_csm_contour(wks,south_O3(:,:),res)
        draw(plot)
