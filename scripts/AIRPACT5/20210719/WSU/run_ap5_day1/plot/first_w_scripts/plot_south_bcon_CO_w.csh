...
# 
#   > plot_south_bcon_CO.csh 20120501 24
#
...
# plot_south_bcon_CO.csh 20120501 24 notes:
...
;********************************************************
  ; position and time to read
    ...
    location_description = "Southern Boundary CO" ; some text to describe the location
  ; BEGIN READ IN species from bcon file
        	CO_bcon = fbcon->CO
        	south_CO = CO_bcon(${ihour},:,0:284)
  ; END READ IN bcon species

    south_CO       = south_CO *1000.                  ; convert to ppb
    ; define/modify dimensions and attributes in a way that NCL understands
  ;    south_CO!0 = "time"       ; first dimension
      south_CO!0 = "lev"       ; first dimension
      south_CO!1 = "west_east" ; second dimension
      south_CO@long_name = "CO MOZART BCON"
      south_CO@units = "ppb"

   ; get dimension sizes
     dimsiz = dimsizes(south_CO)
...
;********************************************************
;CO
  wks = gsn_open_wks("ps" ,"$PSDIR"+"/airpact5_BCON_south_CO_" + "w_$ahour")          ; ps,pdf,x11,ncgm,eps
  gsn_define_colormap(wks,"BlGrYeOrReVi200")           ; select color map
      res@lbTitleString         = "(" + south_CO@units + ")"
          plot  = gsn_csm_contour(wks,south_CO(:,:),res)
        draw(plot)
