...
# 
#   > plot_north_bcon_PM25_flat.csh 20120501 24
#	test 0923_01 is to go back to above ground only
#	#
...
setenv ihour $ihour
setenv ahour $ahour
cat > $NCLSCRIPTS/north_bcon_flat.ncl <<EOF
...
;      h_top_agl!0 = "time"
  ...
  ; height above ground level for each cell's top and bottom
    h_bound = new((/mz+1,my/),typeof(h_top))
       h_bound(0,:) = 0.0 ; lev, row
    do ilevel  = 1, mz
       h_bound(ilevel,:) = h_top(ilevel-1,:)
    end do
...
;          res@tiYAxisString        = "Height Above Sea Level (km)"
          res@tiYAxisString        = "Height Above Ground Level (km)"
;          res@sfYArray              = h_bound_asl(:,:)
          res@sfYArray              = h_bound(:,:)
...
EOF
# end of north_bcon_flat.ncl

       ncl $NCLSCRIPTS/north_bcon_flat.ncl
...
# convert to gif files
  set GIFDIR     = $indir/IMAGES/bcon/hourly/gif_flat
  mkdir -p $GIFDIR
  cd $PSDIR
  foreach psfile ( *.ps )
...
#/bin/rm -f $NCLSCRIPTS/north_bcon_flat.ncl
exit
