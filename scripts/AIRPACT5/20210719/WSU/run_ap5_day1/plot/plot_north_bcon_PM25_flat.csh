...
setenv ihour $ihour
setenv ahour $ahour
cat > $NCLSCRIPTS/north_bcon_ASL.ncl <<EOF
...
;       h_bound_asl(ilevel,:) = h_top_asl(ilevel-1,:)
       h_bound_asl(ilevel,:) = h_top_asl(ilevel-1,:)  + HT(ilevel)
    end do
...
EOF
# end of north_bcon_ASL.ncl

       ncl $NCLSCRIPTS/north_bcon_ASL.ncl
...
# convert to gif files
  set GIFDIR     = $indir/IMAGES/bcon/hourly/gif_asl
  mkdir -p $GIFDIR
  cd $PSDIR
  foreach psfile ( *.ps )
...
#/bin/rm -f $NCLSCRIPTS/north_bcon_ASL.ncl
exit
