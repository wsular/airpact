...
begin
  ...
  ; must include ".nc" in NCL script)
    DRYFILE = addfile ( "$indir" + "/DRYDEP_monthlysum_${1}.ncf.nc","r")
    WETFILE = addfile ( "$indir" + "/WETDEP1_monthlysum_${1}.ncf.nc","r")
...
ncl $NCLSCRIPTS/monthly_deposition.ncl

# convert to gif files
  set GIFDIR     = $indir/IMAGES
  mkdir -p $GIFDIR
  cd $PSDIR
  foreach psfile ( *.ps )
