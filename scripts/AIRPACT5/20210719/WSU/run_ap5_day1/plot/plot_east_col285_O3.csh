...
# 
#   > plot_east_col285_O3.csh  20120501 24
#
...
# plot_east_bcon_O3.csh 20120501 24 notes:
...
# set columm for Easternmsot column 
   set ICOL = 285
...
setenv ihour $ihour
setenv ahour $ahour
cat >! $NCLSCRIPTS/east_conc.ncl <<EOF
...
fconc = addfile ("$indir" + "/CCTM/CONC_" + "$YEARMMDD" + ".ncf.nc", "r")
...
;********************************************************
  ; position and time to read
    icol= $ICOL - 1 ;  284 ; i.e., locate Eastmostcolumn from the south-west corner (NCL counter starts from zero)
    location_description = "Eastern Boundary O3" ; some text to describe the location
  ; BEGIN READ IN species from conc file
        	O3_conc = fconc->O3
        	east_O3 = O3_conc(${ihour},:,:,icol)
  ; END READ IN conc species

    east_O3       = east_O3 *1000.                  ; convert to ppb
    ; define/modify dimensions and attributes in a way that NCL understands
  ;    east_O3!0 = "time"       ; first dimension
      east_O3!0 = "lev"       ; first dimension
      east_O3!1 = "south_north" ; second dimension
      east_O3@long_name = "Ozone MOZART BCON"
      east_O3@units = "ppb"

   ; get dimension sizes
     dimsiz = dimsizes(east_O3)
...
;********************************************************
;OZONE
  wks = gsn_open_wks("ps" ,"$PSDIR"+"/airpact5_BCON_east_O3_" + "$ahour")          ; ps,pdf,x11,ncgm,eps
  gsn_define_colormap(wks,"BlGrYeOrReVi200")           ; select color map
      res@lbTitleString         = "(" + east_O3@units + ")"
          plot  = gsn_csm_contour(wks,east_O3(:,:),res)
        draw(plot)
...
EOF
# end of east_conc.ncl

       ncl $NCLSCRIPTS/east_conc.ncl
...
# /bin/rm -f $NCLSCRIPTS/east_conc.ncl
exit
