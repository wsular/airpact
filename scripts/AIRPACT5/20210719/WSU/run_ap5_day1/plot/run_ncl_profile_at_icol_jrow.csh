...
# 
#   > run_ncl_profile_at_icol_jrow.csh YYYYMMDD ICOL JROW
#
...
#
# This C shell script creates hourly gif files of Profile curtain plots based on AIRPACT-5
...
# > create and run ncl script hour by hour ----------------------------
	...
	setenv IHOUR $ihour
	setenv AHOUR $ahour
cat > $NCLSCRIPTS/profile_at_${ICOL}_${JROW}.ncl <<EOF
...
;********************************************************
  ; position and time to read
    ...
    jrow = $JROW - 1 ; because index in ncl ranges from 0...
  ; Longitude of ID/WA Border is -117 and icol is ~155 
  ; ozone & co concentration
    o3      = fconc->O3(itime,:,jrow,icol)*1000.    ; time,level,j,i
    co      = fconc->CO(itime,:,jrow,icol)*1000.    ; time,level,j,i    
  ...
  ; pressure at mid-level
    pres      = fmet->PRES(itime,:,jrow,icol)         ; time,level,j,i
    ; define/modify dimensions and attributes in a way that NCL understands
    ;  pres!0 = "time"
      pres!0 = "lev"
    ;  pres!1 = "south_north"
  ...
  ; layer top height above ground
    h_top_agl    = fmet->ZF(itime,:,jrow,icol)/1000.         ; time,level,j,i
    ; define/modify dimensions and attributes in a way that NCL understands
    ;  h_top_agl!0 = "time"
      h_top_agl!0 = "lev"
...
;********************************************************
  ; ozone
    location_description = "Profile Curtain Plot for Col $ICOL ROw $JROW "
    res@tiMainString         = "Ozone " + location_description
    wks = gsn_open_wks("ps" ,"$PSDIR"+"/airpact5_Profile_${ICOL}_${JROW}_o3_curtain_" + "$ahour")
...
EOF
# end of profile_at_${ICOL}_${JROW}.ncl

       ncl -Q $NCLSCRIPTS/profile_at_${ICOL}_${JROW}.ncl
       @ ihour ++
end #while
...
#/bin/rm -f $NCLSCRIPTS/profile_at_${ICOL}_${JROW}.ncl
exit
