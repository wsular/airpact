...
#				See flag REDATE
#  20191002      Joe Vaughan	Yikes!  Stale WACCM files can be a problem if they were grabed when not final/static.
#				Scrub out WACCM files each time and reacquire all required files.
#  20191101      Joe Vaughan    New version to grab WACCM files that used QFED fire emissions instead of FINN.

#  README from here: https://www.acom.ucar.edu/acresp/MODELING/mz4_output/waccm/README.txt
#  Files with a nomenclature of f.e21.FWSD.f09_f09_mg17.forecast.001.cam.hX.YYYY-MM-DD-00000.nc are for WACCM output utilizing FINN emissions.
#  Files with a nomenclature of f.e21.FWSD.f09_f09_mg17.forecast.002.cam.hX.YYYY-MM-DD-00000.nc are for WACCM output utilizing QFED emissions.
...
#
#  NOTE: As of February 8th 2019, no new cam.h0 outputs are being stored to this server.  However, all cam.h0 files produced before this date are
#  still being made available.
#
#  NOTE: As of August 13th, 2019, WACCM forecasts should (hopefully) be back on a routine schedule!
#
...
set TRUE = 0
set FALSE = 1
set REDATE = $FALSE  # REDATE set to false
...
#  set waccmfile1 = f.e22.beta02.FWSD.f09_f09_mg17.cesm2_2_beta02.forecast.001.cam.h3.${YEAR}-${MONTH}-${DAY}-00000.nc 
#  set waccmfile2 = f.e22.beta02.FWSD.f09_f09_mg17.cesm2_2_beta02.forecast.001.cam.h3.${NYEAR}-${NMONTH}-${NDAY}-00000.nc 
#  set waccmfile3 = f.e22.beta02.FWSD.f09_f09_mg17.cesm2_2_beta02.forecast.001.cam.h3.${NNYEAR}-${NNMONTH}-${NNDAY}-00000.nc 
set waccmfile1 = f.e22.beta02.FWSD.f09_f09_mg17.cesm2_2_beta02.forecast.002.cam.h3.${YEAR}-${MONTH}-${DAY}-00000.nc 
set waccmfile2 = f.e22.beta02.FWSD.f09_f09_mg17.cesm2_2_beta02.forecast.002.cam.h3.${NYEAR}-${NMONTH}-${NDAY}-00000.nc 
set waccmfile3 = f.e22.beta02.FWSD.f09_f09_mg17.cesm2_2_beta02.forecast.002.cam.h3.${NNYEAR}-${NNMONTH}-${NNDAY}-00000.nc 
set waccmfile1 =    f.e21.FWSD.f09_f09_mg17.forecast.002.cam.hX.YYYY-MM-DD-00000.nc
...
# scrub out all required files.  100219 JKV
  rm -fv $waccmfile1
  rm -fv $waccmfile2
  rm -fv $waccmfile3
...
# If all three necessary files exist, proceed to catenate required hours from each file.
# If files do not all exist, then  set REDATE flag for redate of previous days BCON file for today.
...
if ( -e $waccmfile ) then 
  ls -lt $waccmfile
  ncdump -h $waccmfile | head
  set tsteps = `ncdump -h $waccmfile | head -3 | tail -1 | cut -f2 -d'(' | cut -c1-2`
  if ( $tsteps != 11 ) set REDATE = $TRUE
endif


if ( $REDATE == $FALSE ) then
...
else # REDATE must be TRUE so redate previous day bcon file for today
echo REDATE is $REDATE
