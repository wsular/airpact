...
# # PYTHON CODE FOR EXTRACTING AIRPACT5 BCON FROM CAM-Chem FILES
...
#month = 7
#day   = 25
#time_units_long = 'days since 2018-07-23 00:00:00' 
...
# # SECTION 4 READ IN AND REPORT ON CAM-Chem DATA
print "SECTION 4 READ IN AND REPORT ON CAM-Chem DATA "

# CAM-Chem FILES FROM NCAR ARE SOURCE OF NEW GAS AND AEROSOL BOUNDARY CONDITIONS
# CAM-Chem files are grabbed and prepared by cshell scripting.
# CAM-Chem files set up file locations: CAM-Chem files for boundary conditions input
waccm_file_dir = "/data/lar/projects/airpact5/misc/WACCM_for_BCON/"
waccm_file_name = 'camchem_forecast_{y}-{m}-{d}.nc'.format(m=m, y=y, d=d)
...
#read in CAM-Chem file for inputs for BCON
...
#print waccm_hybm
#print time
waccm_ntime = 10 # Timnes steps to process from CAM-Chem
waccm_nlev = 56 # 88
waccm_nlat = 13
waccm_nlon = 15
waccm_nchars = 8
...
# CAM-Chem numbers variable names
waccm_numbers = ['num_a1', 'num_a2', 'num_a3']  # CAM-Chem file contains no 'num_a4'
...
# CAM-Chem aerosols variable names
...
#  CAM-Chem gas phase species variable names
#  CAM-Chem gas phase species are in moles/mole air, so straight conversion to ppbv is just multiplication by 10^6.
...
waccm_gases = ['ALKNIT', 'BENZENE', 'BIGALK', 'BIGENE', 'C2H2', 'C2H4', 'C2H5OH', 'C2H6', 'C3H6', 'C3H8', 'CH3COCH3', 
               ...
               'NH3', 'NH4', 'NO', 'NO2', 'NOA', 'O3', 'O3S', 'ONITR', 'PAN', 'PBZNIT', 'PHENOL', 'SO2', 'TERPNIT', 
               'TOLUENE', 'XYLENES']
# create empty dictionary structures for CAM-Chem gases, aerosols, and number variables.
waccm_data_gas = {} 
waccm_data_aerosol = {} 
waccm_data_number = {} 
...
for gas in waccm_gases:
    ...
    #print (waccmgrid.variables[gas][7,55,0,0])
    print 'AT 0,0:', waccm_data_gas['waccm_{}'.format(gas)][7,55,0,0]
    

# # SECTION 5 CALCULATE PRESSURE THROUGHOUT CAM-Chem SPACE 
# # note that level index runs from top-of_atmosphere to surface.
print "SECTION 5 CALCULATE PRESSURE THROUGHOUT CAM-Chem SPACE "

# CAM-Chem -- calculation of Pressure at layer midpoints [Pa] 
...
# ## COMPUTE: C_in_Z array for conversion of CAM-Chem  AEROSOL
# ## 		from Kg/Kg to micrograms/meter-cubed,
# ## and COMPUTE N_in_Z array for conversion of CAM-Chem numbers from #/Kg to #/m3
# ## and COMPUTE UGPM3_of_GMR (meaning ug/m3 for Gas Mixing Ratio), to convert NH4 from [ppmv] to [ug/m3] for ANH4J.
# ## N.B. While NH4 is [mol/mol] in the CAM-Chem file, it was converted to [ppbv] when it was read in.
# ## N.B. CAM-Chem level index runs from top-of_atmosphere to surface.

# Compute conversion factor C_in_Z for converting CAM-Chem aerosol in Kg/Kg to micrograms/cubic-meter, a.k.a. ug/m3.
    ...
    for levdx in range(waccm_nlev):
        ...
        for latdx in range(waccm_nlat):
            ...
            for londx in range(waccm_nlon):
                ...
                if ( levdx  == 55 and latdx == 0 and londx == 0):
...
print "C_in_Z check:",C_in_Z[7,(0, 10, 20, 30, 40, 50, 55),0,0]  
print "N_in_Z check:",N_in_Z[7,(0, 10, 20, 30, 40, 50, 55),0,0] 
print "UGPM3_of_GMR:",UGPM3_of_GMR[7,(0, 10, 20, 30, 40, 50, 55),0,0] 
...
#_plt.show()
#print "Conversion factors plotted over CAM-Chem layers at a corner. On X-axis, ToA is far left, Sea Level on far right."
#print "GREEN:  N_in_Z(Z)/10**9 at a corner"
#print "YELLOW:  N_in_Z(Z) at a corner"


# # SECTION 6 READ THE CAM-Chem AEROSOLS
print "SECTION 6 READ THE CAM-Chem AEROSOLS  "

# Load the CAM-Chem aerosol species into a dctionary structure
#compute the AIRPACT-units version of the CAM-Chem aerosol species 
...
for aerosol in waccm_aerosols:
    ...
    print waccm_data_aerosol['waccm_{}'.format(aerosol)][7,55,12,14]

# # SECTION 7 PLOT CAM-Chem AEROSOLS as read
print "SECTION 7 DO NOT PLOT THE CAM-Chem AEROSOLS  "
...
#print "Aerosols as kg/kg plotted over CAM-Chem layers.  On X-axis, ToA is far left, Sea Level on far right."
...
# # SECTION 8 Convert the CAM-Chem AEROSOLS in place.
print "SECTION 8 Convert the CAM-Chem AEROSOLS IN PLACE"
print "SECTION 8.1 Convert the CAM-Chem AEROSOLS IN PLACE"

# compute the AIRPACT version in units [ug/m3] for the CAM-Chem aerosol species 
...
aer_sum_NE = 0.0
for aerosol in waccm_aerosols:
    print aerosol, "results at SW Corner as ug/m3: ", waccm_data_aerosol['waccm_{}'.format(aerosol)][0,55,0,0]
    print aerosol, "results at  NE Corner as ug/m3: ", waccm_data_aerosol['waccm_{}'.format(aerosol)][0,55,12,14]
    aer_sum_SW += waccm_data_aerosol['waccm_{}'.format(aerosol)][0,55,0,0]
    aer_sum_NE += waccm_data_aerosol['waccm_{}'.format(aerosol)][0,55,12,14]
print "Sum of Aerosols at SW surface corner", aer_sum_SW
print "Sum of Aerosols at NE surface corner", aer_sum_NE

print "SECTION 8.2 Convert the CAM-Chem NH4 gas to BCON ANH4J aerosol, in place"
WACCM_NH4 = 'NH4'
aerosol = WACCM_NH4
BCON_NH4 = 'ANH4J'
...
print "Convert CAM-Chem:", WACCM_NH4, " to BCON: ", BCON_NH4
...
# # SECTION 9 PLOT CAM-Chem AEROSOLS after conversion
print " SECTION 9 PLOT CAM-Chem AEROSOLS after conversion"
...
#print "Aerosols as ug/m3 plotted over CAM-Chem layers.  On X-axis, ToA is far left, Sea Level on far right."
...
# # SECTION 10 Load, plot, convert and replot CAM-Chem number species
print " SECTION 10 Load, plot, convert and replot CAM-Chem number species"

# Load the CAM-Chem numbers into the dictionary structure
for number in waccm_numbers:
    print number # this lists each aerosol name
    waccm_data_aerosol['waccm_{}'.format(number)] = np.float64(waccmgrid.variables[number][:,:,:,:]) 
    print waccm_data_aerosol['waccm_{}'.format(number)][:,55,12,14]
...
#_plt.show()
#print "Numbers read in as #/Kg plotted over CAM-Chem layers.  On X-axis, ToA is far left, Sea Level on far right."
#print "BLACK:  waccm_num_a1(Z) at a corner"
#print "GREEN:  waccm_num_a2(Z) at a corner"
#print "YELLOW:  100* waccm_num_a3(Z) at a corner"


# #COMPUTE the AIRPACT-units version of the CAM-Chem number species, in place

# compute the AIRPACT version of the CAM-Chem number species.
...
#_plt.plot(range(waccm_nlev), (100 * waccm_data_aerosol['waccm_num_a3'][tdx, :, 12,14]), marker='*', color='yellow')
#_plt.show()
#print "Numbers as #/m3 plotted over CAM-Chem layers.  On X-axis, ToA is far left, Sea Level on far right."
#print "BLACK:  waccm_num_a1(Z) at a corner"
#print "GREEN:  waccm_num_a2(Z) at a corner"
#print "YELLOW:  100* waccm_num_a3(Z) at a corner"


# # SECTION 11 (already Loaded) so plot, convert and replot CAM-Chem Gas species
print "SECTION 11 (already Loaded) so plot, convert and replot CAM-Chem Gas species"
...
#_plt.show()
#print "CAM-Chem gases as moles/mole over CAM-Chem layers.  On X-axis, ToA is far left, Sea Level on far right."
#print "BLACK:  waccm_CH4 * 100 at a corner"
#print "GREEN:  waccm_CO at a corner"
#print "YELLOW: waccm_O3 * 100 at a corner"


# # SECTION 11.5  does CONVERSION OF GAS PHASE SPECIES FROM CAM-Chem UNITS OF MOLES/MOLE TO PPMV: read & convert in place.
print "SECTION 11.5  CONVERSION OF GAS PHASE SPECIES FROM CAM-Chem UNITS OF MOLES/MOLE TO PPMV"

# create list of the CAM-Chem gas phase species
...
#  CAM-Chem gases' molecular weights
...
waccm_gas_molwt  = [133.141, 78.1104, 72.1438, 56.1032, 26.0368, 28.0516, 46.0658, 30.0664, 42.0774, 44.0922, 58.0768,
                    32.0400, 48.0394, 16.0406, 28.0104,
                                                 108.136, 62.1324, 34.0136, 63.0123, 79.0117, 133.100, 74.0762, 
                    70.0878, 72.1026, 147.085, 70.0878, 44.0129, 108.010,  17.031, 18.039, 30.0061, 46.0055, 119.074, 
                    47.9982, 47.9982, 133.100, 121.048, 183.118, 94.1098, 64.0648, 215.240, 92.1362, 106.162]
...
for gas in waccm_gases:
    ...
    waccm_data_gas['waccm_{}'.format(gas)] = 10**6 * np.float64(waccmgrid.variables[gas][:,:,:,:]) 
    #print waccm_data_gas['waccm_{}'.format(gas)][7,55,12,14]
...
#_plt.plot(range(waccm_nlev), (100. * waccm_o3), marker='*', color='yellow')
#_plt.show()
#print "CAM-Chem gases as #/m3 plotted over CAM-Chem layers.  On X-axis, ToA is far left, Sea Level on far right."
#print "BLACK:  waccm_CH4 * 100 at a corner"
#print "GREEN:  waccm_CO at a corner"
#print "YELLOW: waccm_O3 * 100 at a corner"
...
# file locations: CAM-Chem files for boundary conditions input
# (done above) waccm_file_dir = "/data/lar/projects/airpact5/misc/CAM-Chem_for_BCON/"
...
BCON_TSTEP_DATES = ( __YYYYDDD__)
BCON_TSTEP_TIMES = ( __HHMMSS__ )
for bcts in range(waccm_ntime):
    print "TS:", bcts, BCON_TSTEP_DATES[bcts], BCON_TSTEP_TIMES[bcts]


# check values of time steps for CAM-Chem and BCON
...
#_plt.plot(range(waccm_nlev), (100 * waccm_data_aerosol['waccm_num_a3'][tdx, :, 12,14]), marker='*', color='yellow')
#_plt.show()
#print "Numbers read in as #/Kg plotted over CAM-Chem layers.  Z on X-axis (abscissa) is inverted, so ToA is far left, Sea Level on far right "
#print "BLACK:  waccm_num_a1(Z) at a corner"
#print "GREEN:  waccm_num_a2(Z) at a corner"
#print "YELLOW:  100* waccm_num_a3(Z) at a corner"


# # SECTION 13 Find Indices for mapping CAM-Chem Cells to BCON Perimeter Cells.
print "SECTION 13 Find Indices for mapping CAM-Chem Cells to BCON Perimeter Cells."

# ## SECTION 13.1 Read in CAM-Chem LAT and LON
print "SECTION 13.1 Read in CAM-Chem LAT and LON"

# READ CAM-Chem file for lat and lon.
...
# ## SECTION 13.2 Set up arrays for indices mapping CAM-Chem to BCON
print "SECTION 13.2 Set up arrays for indices mapping CAM-Chem to BCON"

# Set up arrays for storing indices for CAM-Chem to BCON
...
# ## SECTION 13.3 Find indices mapping CAM-Chem to BCON
print "SECTION 13.3 Find indices mapping CAM-Chem to BCON"

#FOR EACH BCON PERIM LAT AND LAN, FIND THE INDICES FOR MAPPING CAM-Chem CELLS TO THAT BOUNDARY CELL.
...
# ## SECTION 13.4 Plot indices mapping CAM-Chem Lat and Lon into BCON
print "SECTION 13.4 Plot indices mapping CAM-Chem Lat and Lon into BCON"

# DRAW BOX SHOWING THE CAM-Chem INDICES TO ALL BCON PERIMITER LAT/LON CELLS
#_plt.plot(i_lon_w2b, j_lat_w2b, marker='X', color='red')
#_plt.show()
print "CAM-Chem to BCON indices"
...
for ib in range(perim_size):
    print "Lon-match at ib: ", ib, 'BCON:',bcon_lon[ib],  'CAM-Chem', waccm_lon[i_lon_w2b[ib]]
    print "Lat-match at ib: ", ib, 'BCON:',bcon_lat[ib],  'CAM-Chem', waccm_lat[j_lat_w2b[ib]]
...
#_plt.show()
#print "RED: CAM-Chem P(Z) at SW corner, Z on X-axis (abscissa) is inverted, so ToA is far left, Sea Level on far right "
...
#_plt.plot(range(waccm_nlev), 100.0 * waccm_lev[:], marker='X', color='black')
#_plt.show()
#print "RED: CAM-Chem P(Z) at SE corner, Z on X-axis (abscissa) is inverted, so ToA is far left, surface layer on far right "
...
#_plt.plot(range(waccm_nlev), 100.0 * waccm_lev[:], marker='X', color='black')
#_plt.show()
#print "CAM-Chem P(Z) at NE corner, Z on X-axis (abscissa) is inverted, so ToA is far left "
...
#_plt.plot(range(waccm_nlev), 100.0 * waccm_lev[:], marker='X', color='black')
#_plt.show()
#print "CAM-Chem P(Z) at NW corner, Z on X-axis (abscissa) is inverted, so ToA is far left "
...
# ## SECTION 14.2 Explore BCON topo heights & pressure vs CAM-Chem surface pressure and lowest level pressure.
print "SECTION 14.2 EXPLORE BCON TOPO HEIGHTS & PRESSURE vs CAM-Chem SURFACE PRESSURE AND LOWEST LEVEL PRESSURE"

# Explore BCON topo heights & pressure vs CAM-Chem surface pressure and lowest level pressure.
...
#    print ib, "B_HT:",bcon_ht[ib],"B_P:",bconmet_pres[0,0,ib],"  W_PS:", waccm_PS[0,j_lat_w2b[ib],i_lon_w2b[ib]], " W_PZ:",P_in_Z[0,55,j_lat_w2b[ib],i_lon_w2b[ib]]
 


# Correlation coefficient calculation.... BCON to P_in_Z

print "Correlation of BCON surface pressure to matching CAM-Chem P_in_Z in bottom layer"
np.corrcoef((bconmet_pres[0,0,:]),(P_in_Z[0,55,j_lat_w2b[:],i_lon_w2b[:]]))



# Correlation coefficient calculation.... BCON to PS

print "Correlation of BCON surface pressure to matching CAM-Chem PS in bottom layer"
np.corrcoef((bconmet_pres[0,0,:]),(waccm_PS[0,j_lat_w2b[:],i_lon_w2b[:]]))


# ## SECTION 14.3 Use pressure comparisons between BCON and P_of_Z to set indices for mapping CAM-Chem level to BCON level.
print "SECTION 14.3 USE PRESSURE COMPARISONS BETWEEN BCON and P_of_Z TO SET INDICES FOR MAPPING CAM-Chem LEVEL TO BCON LEVEL"

# USE PRESSURE COMPARISONS BETWEEN BCON and P_of_Z to set indices for mapping CAM-Chem level to BCON level.
# TEST: perim_size = 1
# TEST: waccm_ntime = 1
# Create an empty array for indices into CAM-Chem by BCON perimiter and BCON level.
time_begin = time.time()

# waccm_ntime = 1 # TEST
# perim_size = 1090 # TEST 

#set up verical array of depth of CAM-Chem to hold pressure deltas.
...
for tdx in range(waccm_ntime):
    #for tdx in range(waccm_ntime):
    print 'For CAM-Chem Time index:', tdx
    for ib in range(perim_size):
...
#            min_delta_P = -10**6
            ...
            for top_down_levdx in range(waccm_nlev):
                #print "........CAM-Chem top_down_levdx:", top_down_levdx
                ...
                if ( delta_P > last_delta_P ):
                    ...
                    # print "..........CAM-Chem top_down_levdx:", top_down_levdx
                    # print "..........CAM-Chem pressure match of: ",last_P_of_Z 
...
# # SECTION 15 Transfer Aerosols and Numbers from CAM-Chem space to BCON space, using cross-walk by Yunha.
print "SECTION 15 TRANSFER AEROSOLS and NUMBERS FROM CAM-Chem SPACE TO BCON SPACE"

# Convert CAM-Chem MAM4 aerosol species (list is in aerosol) to AERO6 (list is in BCON_aerosol),
# while using indices for copying from CAM-Chem vertical layers to 
...
if ( not bcon_zeroed ):
    ...
    #ANH4J  comes from CAM-Chem gases and was converted in SECTION 8.2 to ug/m3
    ...
    #NUMACC     CONVERSION FROM units [#/kg] to [#/m3] for Accumulation Mode was done in sect. 10, above.
                ...
                # num_a4 not on CAM-Chem files                 + 1.0 * waccm_data_aerosol['waccm_num_a4'][tdx,W_levdx,W_lat,W_lon] )
    ...
    # species by species copy, splittng and combining
    #ANH4J is calculated from CAM-Chem gas phase NH4.  See note relflecting emails with Louisa Emmons. 
...
# # SECTION 16  Transfer & Translate CAM-Chem gas phase species from CAM-Chem space to BCON  space.
print "SECTION 16  TRANSFER & TRANSLATE CAM-Chem GAS PHASE SPECIES FROM CAM-Chem SPACE TO BCON SPACE"

# Convert CAM-Chem gas species (list is in WACCM_gases) to CB06 (list is in BCON_aerosol),
# while using indices for copying from CAM-Chem vertical layers to 
...
if ( not bcon_zeroed ):
    ...
    for tdx in range(waccm_ntime):
        ...
        for ib in range(perim_size):
            ...
            for ilev in range(bconmet_nlev):    #Vertical traverse bottom to top of BCON levels   
                #print "......For BCON level index:", ilev
                # bc_lev_from_waccm_lev[ib,ilev], j_lat_w2b[ib], i_lon_w2b[ib] maps perimeter and level from CAM-Chem to BCON.
                W_levdx = bc_lev_from_waccm_lev[tdx,ib,ilev]
                W_lat = j_lat_w2b[ib]
                W_lon = i_lon_w2b[ib]
                # species by species copy, splittng(?) and combining.
