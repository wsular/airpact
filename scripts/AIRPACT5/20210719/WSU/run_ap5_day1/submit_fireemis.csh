...
#  if current time is after 9:30 PM (21:30) delay until 11:45 PM (23:45).
    set nowis = `date | cut -d' ' -f4 | cut -f1,2 -d':' | tr -d ':'`
    set not_until = 2345  # time to start fire emissions scripting.
    set mcip_by  = 2130   # time typical for MCIP files arriving and ssh of masterall.csh starting run. 
    if ( $nowis < $mcip_by ) then 
         echo Time of run is not in expected window of 9:30 PM to Midnight, for operational runs, so run it.
         set a_option = ''
    else
	 echo Time of run is in expected operational window of 9:30 PM to Midnight, so delay fire emissons call.
         set a_option = "-a $not_until" 
    endif
...
# job submission -------------------------------------------------------
  # create qsub script
    ...
    if ( $?runid0 ) then # if waiting on a job to finish
       ...
       set qsubreturn = `qsub -V $a_option $qsubfile`
    endif
