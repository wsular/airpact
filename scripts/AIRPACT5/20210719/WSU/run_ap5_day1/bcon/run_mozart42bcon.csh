...
#  2018-??-??    Joe Vaughan   Changed logic to use monthly profiles.
...
# directories and files ------------------------------------------------
  ...
  set PREVBCON    = $AIRROOT/${previousdayyear}/${previousday}00/BCON/output/bcon_cb05_${previousday}.ncf 
  
  cd $indir

###  Force link for monthly file from 2015.  JKV 020218
...
# download MOZART-4 file if necessary ----------------------------------
  ...
  if ( ! -e $inputfile ) then
     ...
     if ( -e $mz4file ) then
        ...
        gzip $mz4file
     else # missing today's mz4 file, use yesterday's mz4 file
        ...
        if ( -e $inputfileprevious ) then
           ...
           cp $inputfileprevious $inputfile
        else if ( -e $PREVBCON && ! -l $PREVBCON ) then
	   cd $outdir 
	   echo "***   warning no previous day's file for MOZART-4 "
           echo "--->> use previous day's BCON file and redate it using m3tshift JKV 111617 "
#	   setenv INFILE /data/lar/projects/airpact5/AIRRUN/${previousdayyear}/${previousday}00/BCON/output/bcon_cb05_${previousday}.ncf
	   setenv INFILE ${PREVBCON}
	   set YYYYDDD = `ncdump -h $INFILE | grep SDATE | cut -c 12-18`
	   @ YYYYDDDp1 = $YYYYDDD + 1
	   echo Use m3tshift to update BCON for $YYYYDDD to use for $YYYYDDDp1
	   cat >! bcon_tshft.txt << EOF
INFILE
...
EOF
	   ...
	   cat bcon_tshft.txt | m3tshift >! $AIRLOGDIR/bcon/tshift.log
	   if ( $status == 0 && -e OUTFILE ) mv -f OUTFILE $outputfile
  	   echo ' finished in run_mozart42bcon.csh by m3tshift to produce needed file.' 
	   ls -lht *
	   exit (0)	   
        else
           echo "--->> use monthly profile "
