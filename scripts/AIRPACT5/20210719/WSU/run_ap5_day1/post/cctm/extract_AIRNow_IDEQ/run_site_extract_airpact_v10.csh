...
#> check environment variable ------------------------------------------
   ...
   set BASE = ${PBS_O_WORKDIR}/post/cctm/extract_AIRNow_IDEQ
   echo BASE $BASE 
...
#
#   	2) Use sed to apply edits to create new aqsites file.
 	setenv NEWAQSID  $BASE/AQSID_PUWA.csv 
...
#   	3) Call v10 version of extraction program.
	...
	echo "python3 ${BASE}/site_extract_v10.py $previousday  >&! $AIRLOGDIR/post/log04_post_xtr4airnow_IDEQ.txt"
	python3 ${BASE}/site_extract_v10.py $previousday  >&! $AIRLOGDIR/post/log04_post_xtr4airnow_IDEQ.txt
	set pystatus = $status
	if ( $pystatus == 0 ) then
