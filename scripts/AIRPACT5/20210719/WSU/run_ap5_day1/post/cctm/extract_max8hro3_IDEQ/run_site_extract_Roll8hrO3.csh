...
#
#   Responding to IDEQ, Rong Li, request for 8hrO3max  for 2020 at two sites, Pocatello and Idaho Falls.
...
#   	3) Call v10 version of extraction program.
	...
	ls -lt  $BASE/site_extract_Roll8hrO3.py 
        rm -f $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt 
        touch $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt 
	# do I need the argument for the py code?
	echo "python3 ${BASE}/site_extract_Roll8hrO3.py $previousday  >&! $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt"
	python3 ${BASE}/site_extract_Roll8hrO3.py $previousday  >&! $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt
	set pystatus = $status
	if ( $pystatus == 0 ) then
	   echo Status after python script is  $pystatus
	   echo Report completion to log file
	   echo "--->> Normal Completion of program site_extract" >> $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt
	   echo " status on echo to $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt is $status"
	   echo "Successful end" >> $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt
	   echo "Date and time:", `date` >> $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt
	   echo " " >> $AIRLOGDIR/post/log04_post_xtr4ideq_IDEQ.txt
        else
