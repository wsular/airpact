#!/bin/csh -f

# Serena H. Chung
# 2015-09-10

#   ./run_combine_v2.csh 20120701

#> location of the combine program ---------------------------------
   set prgdir = ~airpact5/AIRHOME/build/combine/bld
   set exec = combine.exe

#> initialize exit status ----------------------------------------------
   set exitstat = 0

#> check argument & set date strings  ----------------------------------

   if ( $#argv == 1 ) then
      set currentday = $1
      set YEAR  = `echo $1 | cut -c1-4`
      set MONTH = `echo $1 | cut -c5-6`
      set DAY   = `echo $1 | cut -c7-8`
   else
      echo 'invalid argument. '
      echo "usage $0 <yyyymmdd>"
      set exitstat = 1
      exit ( $exitstat )
   endif

  #> determine day of the year
     set DOYstring = `juldate $MONTH $DAY $YEAR | cut -d',' -f 2`
     set DOY       = `echo $DOYstring | cut -c5-7`
     set YEARDOY   = $YEAR$DOY

#> during testing 

   if ( ! $?PBS_O_WORKDIR ) then
      set PBS_O_WORKDIR = ~airpact5/AIRHOME/run_ap5_day1
      set AIROUT = /data/lar/projects/airpact5/AIRRUN/${currentday}00
   endif
    
#> directory set-up ----------------------------------------------------

   set BASE    = $PBS_O_WORKDIR/post/cctm/combine
   set  INDIR  = $AIROUT/CCTM
   set OUTDIR  = $AIROUT/POST/CCTM
   if ( ! -d $OUTDIR ) mkdir -p $OUTDIR


   setenv SPECIES_DEF ~airpact5/AIRHOME/run_ap5_day1/post/cctm/combine/spec_def_conc_v2.txt
   setenv INFILE1 $AIROUT/CCTM/ACONC_${currentday}.ncf
   setenv INFILE2 $AIROUT/CCTM/AEROVIS_${currentday}.ncf
   setenv INFILE3 $MCIPDIR/METCRO3D
   setenv INFILE4 $AIROUT/CCTM/AERODIAM_${currentday}.ncf
   setenv INFILE5 $MCIPDIR/METCRO2D
   setenv OUTFILE $OUTDIR/combined_${currentday}.ncf
   if ( ! -e $INFILE1 ) then 
      echo $INFILE1 not found  
      exit(1)          
   else if ( ! -e $INFILE2 ) then 
      echo $INFILE2 not found  
      exit(1)          
   else if ( ! -e $INFILE3 ) then 
      echo $INFILE3 not found  
      exit(1)          
   else if ( ! -e $INFILE4 ) then 
      echo $INFILE4 not found  
      exit(1)          
   else if ( ! -e $INFILE5 ) then 
      echo $INFILE5 not found  
      exit(1)          
   endif
   if ( -e $OUTFILE ) /bin/rm -f $OUTFILE

  #echo attempting to create $OUTFILE
   set logfile = $AIROUT/LOGS/post/log01_post_combine.txt
   $prgdir/${exec} >&! $logfile

   echo "checking" $logfile
   set tmp_str1 = `tail -4 $logfile`
   set tmp_str2 = `echo "$tmp_str1[1-6]"`
   if ( `echo $tmp_str2` == `echo "--->> Normal Completion of program COMBINE"` ) then
      set tmp = `grep -i error $logfile`
      if ( $#tmp == 0 ) then
          echo "  ok; created" $OUTFILE
      else
         echo "  error in" $logfile
         echo $tmp
         set run_status = 1
         exit($run_status)
      endif
   else
      echo "      error running combine"
      echo `tail -4 $logfile`
      echo $tmp_str2
      set run_status = 1
      exit($run_status)
   endif

