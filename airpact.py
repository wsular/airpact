...
File dependenices:
    This Python module must be stored in the same directory as
        CRFs.csv     - Concentration-Response Factors from Matt Kadlec
...
Written by Von P. Walden, Washington State University
           ...
           Updated: 16 October 2020 - Changed to smokeCARE (vpw)
...
class smokeCARE:
    ...
    Example usage:
        ...
        from airpact import smokeCARE
        case = smokeCARE()
        p = case.data['Asthma symptoms onset'].sel(age=25).hvplot.quadmesh(x='longitude', y='latitude', width=1200, height=800, cmap='viridis', title='Increase in Asthma symptoms onset')
        hvplot.show(p)
        
    
    Output:
        smokeCARE - an xarray dataset containing mapping data of the health 
    ...
    def __init__(self):
        ...
        # ....Read the Concentration-Response Factors (CRFs)
        ...
        self.data.attrs['Authors']                = u"AIRPACT5 forecasts from Joe Vaughan, Washington State University, Concentration-Response Factors from Matt Kadlec, WA Dept of Ecology"
